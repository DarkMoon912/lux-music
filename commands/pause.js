...
module.exports = {
    name: "pause",
    description: "Приостановить музыку в голосовом канале.",
    usage: "",
    permissions: {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (player.paused) return client.sendTime(message.channel, "Музыка уже приостановлена!");
        player.pause(true);
        let embed = new MessageEmbed().setAuthor(`Пауза песни`, client.config.IconURL).setColor("#303136").setDescription(`Напишите \`${GuildDB.prefix}resume\` чтобы продолжать играть!`);
     ...
     */
        run: async (client, interaction, args, { GuildDB }) => {
            ...
            if (player.paused) return client.sendTime(interaction, "Музыка уже поставлена на паузу!");
            player.pause(true);
            client.sendTime(interaction, "Поставлен на паузу!");
        },
    },
};
